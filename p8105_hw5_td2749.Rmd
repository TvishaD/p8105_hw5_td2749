...
sim_results_df = 
  expand_grid(
    mu = c(0, 1, 2, 3, 4, 5, 6),
    iteration = 1:10) %>% 
  mutate(broom::tidy(t.test(rnorm(n = 30, mean = mu, sd = 5), mu = mu, conf.level = 0.95))) %>% 
  select(mu, iteration, estimate, p.value) %>% 
  rename(obs_mu = estimate, p_value = p.value)
```

```{r}
t_test_func = function(mu) {
  ...
  sim_data %>%
    summarize(broom::tidy(t.test(x, mu = mu, conf.level = 0.95, alternative = "two.sided"))) %>% 
    select(estimate, p.value) %>% 
    rename(obs_mu = estimate, p_value = p.value)
